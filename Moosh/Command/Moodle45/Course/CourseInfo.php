...
class CourseInfo extends MooshCommand
{
    ...
    private $logsnumber;
    private $data;
    private $questionbankquestions = 0;
    ...
    public function execute()
    {
        ...
        // Get # of questions from the question bank assigned directly to the course context.
        $sql = "SELECT COUNT(DISTINCT qbe.id) AS count FROM {question_bank_entries} qbe JOIN {question_categories} qc ON qc.id = qbe.questioncategoryid JOIN {context} ctx ON ctx.id = qc.contextid WHERE ctx.contextlevel = ? AND ctx.instanceid = ?";
        $params = [CONTEXT_COURSE, $courseid];

        $questions = $DB->get_record_sql($sql, $params);
        $this->questionbankquestions = $questions ? (int)$questions->count : 0;
    ...
    private function aggregateData()
    {
        ...
        $this->data["Number of questions from the question bank for the course"] = $this->questionbankquestions;
