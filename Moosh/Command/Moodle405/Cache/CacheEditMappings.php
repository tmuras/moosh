...
class CacheEditMappings extends MooshCommand {
    ...
    public function execute() {
        ...
        if ($options['application']){
            $data->{'mode_'.\core_cache\store::MODE_APPLICATION} = $options['application'];
        }
        else {
            $data->{'mode_'.\core_cache\store::MODE_APPLICATION} = $defaultconfig[0]['store'];
        }

        if ($options['session']){
            $data->{'mode_'.\core_cache\store::MODE_SESSION} = $options['session'];
        }
        else {
            $data->{'mode_'.\core_cache\store::MODE_SESSION} = $defaultconfig[1]['store'];
        }

        if ($options['request']){
            $data->{'mode_'.\core_cache\store::MODE_REQUEST} = $options['request'];
        }
        else {
            $data->{'mode_'.\core_cache\store::MODE_REQUEST} = $defaultconfig[2]['store'];
        }

        $mappings = array(
            \core_cache\store::MODE_APPLICATION => array($data->{'mode_'.\core_cache\store::MODE_APPLICATION}),
            \core_cache\store::MODE_SESSION => array($data->{'mode_'.\core_cache\store::MODE_SESSION}),
            \core_cache\store::MODE_REQUEST => array($data->{'mode_'.\core_cache\store::MODE_REQUEST}),
        );
        ...
        catch (\core_cache\exception\cache_exception $e) {
            cli_error($e->getMessage());
        }

        echo "Config Mode Mappings:";
        echo "\nMODE_APPLICATION: ".$mappings[\core_cache\store::MODE_APPLICATION][0];
        echo "\nMODE_SESSION: ".$mappings[\core_cache\store::MODE_SESSION][0];
        echo "\nMODE_REQUEST: ".$mappings[\core_cache\store::MODE_REQUEST][0];
