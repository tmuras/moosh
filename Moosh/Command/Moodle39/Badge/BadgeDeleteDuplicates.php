...
class BadgeDeleteDuplicates extends MooshCommand
{
    ...
    public function execute()
    {
        ...
        // Get duplicates.
        $duplicates = $DB->get_records_sql('SELECT ROW_NUMBER() OVER (ORDER BY timecreated, courseid) as rownumber, count(*), timecreated, courseid FROM {badge} WHERE status=0 GROUP BY timecreated, courseid having count(*) > 1');
